...
int main() {
  ...
  int N = (int)S.size();
  // OUT(reversed.substr((N + 1) / 2));
  // OUT(S.substr((N + 3) / 2 - 1));

  if (isPalindrome(S) && isPalindrome(reversed.substr((N + 1) / 2)) && isPalindrome(S.substr((N + 3) / 2 - 1))) {
    OUT("Yes");
  } else {
    OUT("No");
  }
